...
import { ShoppingBag, Search, ShoppingCart, LogOut, Package, ChevronsUpDown } from "lucide-react";
...
export default function StorefrontHeader() {
  ...
  const searchFromUrl = pathname === "/" ? (searchParams.get("q") ?? "") : "";
  ...
  const activeCategory = pathname === "/" ? (searchParams.get("category") ?? undefined) : undefined;
  ...
  return (
    <header className="sticky top-0 z-40 border-b-2 border-border/60 bg-primary text-primary-foreground">
        ...
        <form onSubmit={handleSearch} className="min-w-0 flex-1">
          <div className="relative">
            <Input
              ...
              className="h-10 border-transparent bg-white pr-11 text-foreground shadow-none focus-visible:border-transparent focus-visible:ring-2 focus-visible:ring-white/60"
            />
            <button
              type="submit"
              aria-label="Cari"
      ...
      {/* Persistent category quick-nav — Tokopedia keeps category access one
          click away from every page, not just the homepage. */}
      {!!categories?.length && (
            ...
            <Link
              ...
              className={
                pathname === "/" && !activeCategory
                  ? "shrink-0 rounded-md bg-white/15 px-3 py-1 text-xs font-bold"
                  : "shrink-0 rounded-md px-3 py-1 text-xs font-semibold text-primary-foreground/80 transition-colors hover:bg-white/10 hover:text-primary-foreground"
              }
            >
              Semua
            </Link>
